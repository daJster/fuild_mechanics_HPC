...
def equilibruim_distribution(density_grid) :
    # Calculate the dot product between direction and the velocity
    dot_product = np.einsum('ji, jkl -> ikl', direction, mu(density_grid))
    # Calculate the magnitude squared of the velocity
    magnitude_squared = np.linalg.norm(mu(density_grid), axis=0)**2
    # Apply the formula
    result = W[:, np.newaxis, np.newaxis] * rho(density_grid)[np.newaxis, :, :] * (1 + 3 * dot_product + (9/2) * dot_product**2 - (3/2) * magnitude_squared[np.newaxis, :, :])
    return result

def collision_term(density_grid, relax) :
    return density_grid + relax*(equilibruim_distribution(density_grid) - density_grid) 
...
def create_collision_grid() : 
    ...
    for i in range(50) :
        for j in range(50) :
            density_grid_animate[:, i + offset, j + offset] = [density_grid_animate[k, i + offset, j + offset] + .2 for k in range(9)]
...
if __name__ == "__main__" :
    ...
    print(rho(density_grid).shape, mu(density_grid).shape, direction.shape, W.shape)
    ...
    animate(file='density_grid_collision.mp4', frames=400, collision=collision_function, create_grid=create_collision_grid)